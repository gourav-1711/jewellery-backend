...
const accountSid = "AC332cebda2a6c7483a3df0ea9b708a9b2";
const authToken = "1ca57b92703206b74b77e033fcfb7e3b";
...
module.exports.verifyPhone = async (req, res) => {
  ...
  client.verify.v2
    .services("VA2903b4c298eade93441dd6b7ea22d7b3")
    .verifications.create({ to: phone, channel: "sms" })
    .then((verification) => {
      return res.status(200).json({
        _status: true,
        _message: "Verification sent successfully",
...
module.exports.completePhone = async (req, res) => {
  let { phone, otp } = req.body;

  if (!phone || !otp) {
    return res.status(400).json({
      _status: false,
      _message: "Phone number and OTP are required",
    });
  }

  // Format phone number to match verifyPhone
  ...
  try {
    ...
    return res.status(200).json({
      _status: true,
      _message: "Verification completed successfully",
      _verification_check: verification_check,
      _user: user,
    });
  } catch (error) {
    console.error("Verification error:", error);
    ...
    if (error.code === 20404) {
      return res.status(400).json({
        _status: false,
        _message:
          "Verification not found or expired. Please request a new code.",
      });
    }
    ...
    if (error.code === 60202) {
      return res.status(400).json({
        _status: false,
        _message: "Maximum check attempts reached. Please request a new code.",
      });
    }
    ...
    if (error.code === 60200) {
      return res.status(400).json({
        _status: false,
        _message: "Invalid phone number format.",
      });
    }
