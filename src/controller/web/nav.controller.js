...
exports.navController = async (req, res) => {
  try {
    ...
    const subSubCategories = await subSubCategoryModel.find().lean();
      ...
      const categorySubCategories = subCategories
        .filter(subCat => {
          ...
          if (Array.isArray(subCat.category)) {
            return subCat.category.some(catId => catId.toString() === category._id.toString());
          }
        ...
        .map(subCat => {
          ...
          const subCatSubSubCategories = subSubCategories.filter(subSubCat => {
            ...
            if (Array.isArray(subSubCat.subCategory)) {
              return subSubCat.subCategory.some(subCatId => subCatId.toString() === subCat._id.toString());
            }
            return subSubCat.subCategory?.toString() === subCat._id.toString();
          });
