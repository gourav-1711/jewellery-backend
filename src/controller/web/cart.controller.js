...
module.exports.getCart = async (req, res) => {
    try {
        ...
        const items = await Promise.all(cart.items.map(async (item) => {
            const product = await Product.findById(item.product).select('name price discount_price images');
            const itemTotal = product.discount_price > 0 ? product.discount_price * item.quantity : product.price * item.quantity;
